...
fn read_powers_from_file<E: Engine>(fname: &str) -> Result<TauPowers<E>, DeserializationError> {
    let mut reader = OpenOptions::new()
        .read(true)
        .open(fname)
        .expect(&format!("unable open {} in this directory", fname));
    read_powers(&mut reader)
}

fn read_powers_from_url<E: Engine>(url: &str) -> Result<TauPowers<E>, DeserializationError> {
    let resp = isahc::get(url)?;
    let mut body = resp.into_body();
    read_powers(&mut body)
}

fn read_powers<E: Engine, R: Read>(reader: &mut R) -> Result<TauPowers<E>, DeserializationError> {
    skip_hash(reader);
    let g1p = read_vec::<_, E::G1Affine>(reader, TAU_POWERS_G1_LENGTH, 20)?;
    let g2p = read_vec::<_, E::G2Affine>(reader, TAU_POWERS_LENGTH, 20)?;
    Ok(TauPowers {
        tau_g1: g1p,
        tau_g2: g2p,
    })
...
fn main() {
    ...
    //let zcash_acc = read_powers_from_file::<Bls12>(zcash_file);
    ...
    let filecoin_acc = read_powers_from_url::<Bls12>(filecoin_url);
}
